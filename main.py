...
def create_analysis(
    ...
    if not coords:
        raise HTTPException(
            status_code=400, 
            detail=f"Could not resolve destination location coordinates: '{analysis.destination}'"
        )
...
def create_route_analysis(
    ...
    if not route_safety.get("success", False):
        raise HTTPException(status_code=400, detail=route_safety.get("error", "Route analysis failed"))
